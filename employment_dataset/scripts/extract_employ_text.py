...
# Folders
# -------------------------
raw_folder = os.path.join(BASE_PATH, "employment_dataset", "raw_docs")
txt_folder = os.path.join(BASE_PATH, "employment_dataset", "clauses_raw")
        ...
        if file.endswith(".pdf"):
            ...
            with open(txt_path, "w", encoding="utf-8") as f:
                for para in doc.paragraphs:
                    f.write(para.text + "\n")
            print(f"Converted DOCX: {file} → {txt_path}")

        elif file.endswith(".txt"):
            # Direct copy if raw is already text
            with open(file_path, "r", encoding="utf-8") as src, \
                 open(txt_path, "w", encoding="utf-8") as dest:
                text = src.read()
                dest.write(text)
            print(f"Copied TXT: {file} → {txt_path}")
        ...
        for clause_type, keywords in clauses.items():
            for kw in keywords:
                if kw.lower() in para_lower:
                    ...
                    break  # avoid duplicate matches for same clause_type
    return matches
    ...
    for root, dirs, files in os.walk(clauses_raw_path):
        for file in files:
            if file.endswith(".txt") and file not in ["all_agreements.csv", "extracted_clauses_candidates.csv"]:
                ...
                # 🔹 Better splitting: blank lines, sentence endings, or numbered items
                paragraphs = re.split(r"\n\s*\n|(?<=\.)\s+|(?<=\d\.)\s+", text)
                paragraphs = [p.strip() for p in paragraphs if p.strip()]

                # 🔹 Fallback: if regex fails, take full text
                if not paragraphs:
                    paragraphs = [text.strip()]

                print(f"[DEBUG] {file} → {len(paragraphs)} paragraphs found")
                if len(paragraphs) <= 2:  # suspiciously low
